# Shlok-Superpad
A macropad I fully designed with help from hackclub with 9 keys in a matrix, a rotary encoder, oled screen, and rgb backlighing for each key.

# CAD
I'm screwing direcrly into the plastic with 2.9mm holes for construction of the case. The 2 parts are the top (with the mercedes logo) and the bottom.
<img width="1414" height="986" alt="Screenshot 2026-07-10 155822" src="https://github.com/user-attachments/assets/267b3a3b-1324-4244-bbb7-b0286bbd2d7d" />

# PCB
<img width="1604" height="1093" alt="Screenshot 2026-07-10 160123" src="https://github.com/user-attachments/assets/4bc95bf5-0721-4024-926f-ebbf980ce0fc" />

<img width="953" height="1197" alt="Screenshot 2026-07-10 160159" src="https://github.com/user-attachments/assets/add81646-dff6-4347-a757-066acf6b5fb9" />

# Case
<img width="1929" height="1015" alt="Screenshot 2026-07-10 160324" src="https://github.com/user-attachments/assets/69a7a3a7-9a49-4e77-9f6b-6f62c9ac6e0c" />

# BOM
1 Seeeduino XIAO RP2040

9 MX- style switches

9 DSA keycaps

10 1N4148 Diodes

1 EC11 rotary encoder

9 SK6812 MINI-E LEDs

1 0.91 in OLED 

1 custom pcb

1 3D printed case(2 parts)

4 M3x16mm scews
